...
// own requests; reviewers/admins may see any. Returns null if the request
// doesn't exist OR the caller isn't allowed to see it -- callers should
// treat both cases as "not found" rather than leaking existence.
...
// Backend-enforced list query used by both the reviewer queue and the
// employee's own request list (via GET /api/requests). `scopeUserId` is set
// by the caller for employees (never trust a client-supplied requesterId to
// widen access) and left undefined for reviewers/admins unless they're
// deliberately filtering by requester.
...
// Financial summary per problem_statement.md §8. Definitions:
// - totalRequested: sum of every request that has entered the workflow
//   (i.e. everything except drafts, which are not yet a "request" to anyone
//   but the submitter).
...
// `scopeUserId` restricts the summary to one submitter's own requests (used
// on the employee dashboard); omit it for the org-wide reviewer summary.
