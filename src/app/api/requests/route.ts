...
// GET /api/requests -- paginated, filterable, sortable list of reimbursement
// requests. Backs both the employee's own request list and the reviewer
// queue (problem_statement.md §7, §13, §14).
//
// Query params: status, category, requesterId, dateFrom, dateTo, q, page,
// pageSize, sort (newest | oldest | amount_desc | amount_asc).
//
...
export async function GET(request: NextRequest) {
  ...
  if (role !== "employee" && role !== "reviewer" && role !== "admin") {
    return NextResponse.json(
